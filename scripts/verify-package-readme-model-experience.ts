...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/crm/crm': { kind: 'none', reason: 'Host-side CRM service over the storage seam; it registers no tool, prompt, or session event and owns no model-facing surface.' },
  'packages/crm/crm-console': { kind: 'none', reason: 'Operator-facing HTTP console over ctx.crm; it registers no tool, prompt, or session event, and its only model-visible consequence is data the tool package reads.' },
  'packages/import/session-import-console': { kind: 'none', reason: 'Operator-facing HTTP console over ctx.sessionImport; it registers no tool, prompt, or session event, and its only model-visible consequence is sessions the import tool consumes.' },
